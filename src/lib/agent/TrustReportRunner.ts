...
import { CandidateSeeder } from '../ingest/seeder';

export class TrustReportRunner {
    static async run(options: { dryRun: boolean, enabled: boolean }) {
        const { dryRun: DRY_RUN, enabled: ENABLED } = options;
        const neynar = new NeynarAPIClient(process.env.NEYNAR_API_KEY!);

        console.log(`🔵 TRUST_REPORT START: DryRun=${DRY_RUN}, Enabled=${ENABLED}`);
        ...
        if (totalEligible < threshold) {
            ...
            return { success: false, reason: 'QUALITY_GATE_FAILED', metrics: { totalBaseApps, totalEligible } };
        }

        if (allSelected.length === 0) {
            console.log("⚠️ No candidates selected for Trust Report.");
            return { success: false, reason: 'NO_CANDIDATES_SELECTED' };
        }
        ...
        if (DRY_RUN) {
            ...
            if (!signerUuid) {
                ...
                throw new Error("NEYNAR_SIGNER_UUID missing in production");
            }

            try {
                // PUBLISH TEASER ONLY
                const result: any = await neynar.publishCast(signerUuid, teaserText, { embeds: [] });
                if (result && (result.cast?.hash || result.hash)) {
                    publishedHash = result.cast?.hash || result.hash;
                    console.log(`🚀 PUBLISHED! Hash: ${publishedHash}`);
                } else {
                    console.error("❌ Publish returned unexpected structure.");
                }
            } catch (err) {
                console.error("❌ Publish Failed:", err);
            }
        } else {
            console.log("⏸️  Trust Report disabled by config.");
            // Mark as skipped/disabled logic if needed suitable here
        }
